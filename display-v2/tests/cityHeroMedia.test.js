...
test('inkwash 恒用插画，不查 manifest', () => {
  const resolveAsset = mock.fn(() => ({ url: '/media/inkwash/x.mp4', type: 'video', poster: null }))
...
test('real 空 slug 回退插画', () => {
  const resolveAsset = () => null
  const r = resolveCityHeroMedia({ isReal: true, slug: '', resolveAsset, illustration })
  assert.deepEqual(r, { type: 'image', url: illustration, poster: null, kind: 'illustration' })
})

test('inkwash 无插画返回 null', () => {
  const resolveAsset = () => ({ url: '/media/inkwash/x.mp4', type: 'video', poster: null })
  const r = resolveCityHeroMedia({ isReal: false, slug: 'jinan', resolveAsset, illustration: null })
  assert.equal(r, null)
})
