...
import api from '../../api'
import SectionHeading from '../homepage/SectionHeading.vue'
import DynastyRail from '../homepage/DynastyRail.vue'
import ErrorState from '../homepage/ErrorState.vue'
import SkeletonBlock from '../homepage/SkeletonBlock.vue'
...
const selected = computed(
  () =>
    props.data.find((t) => t.dynasty.id === selectedDynastyId.value) ||
    props.data[0] ||
    null,
)
...
onMounted(async () => {
  await nextTick()
  // 触发 reveal 动画
  const root = document.querySelector('.real-timeline')
  if (root) reveal(root)
})
</script>
