<template>
  <div class="spot-detail" :class="{ 'anime-layout': isAnime }" v-if="spot">
    <!-- BACK BUTTON -->
    ...
    <!-- ANIME THEME LAYOUT (历史曲线页.png replica) -->
    <div class="anime-container animate-fade-in" v-else>
      <div class="spot-split-layout">
        <!-- LEFT COLUMN: Spot Info & Hanging Scroll -->
        <aside class="spot-left-col">
          <div class="spot-title-box">
            <h1 class="spot-name-vertical">{{ spot.name }}</h1>
            <span class="spot-seal" v-if="getSpotData(spot.name).tag">{{ getSpotData(spot.name).tag }}</span>
          </div>
    ...
    series: [{
      ...
      areaStyle: {
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
          { offset: 0, color: 'rgba(142, 53, 46, 0.15)' },
          { offset: 1, color: 'rgba(142, 53, 46, 0.01)' }
        ])
      },
      markPoint: {
        symbol: 'pin',
